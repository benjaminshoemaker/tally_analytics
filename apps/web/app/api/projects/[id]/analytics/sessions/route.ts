...
import {
  getSessionsSummary,
  isSessionsSummarySuccess,
  parseAnalyticsPeriod,
  toDashboardSessionsResponse,
...
export async function GET(
  ...
  const result = await getSessionsSummary({ userId: user.id, projectId, period });
  if (result.status === 'project_not_found') {
    return Response.json({ error: 'Project not found' }, { status: 404 });
  }
  if (!isSessionsSummarySuccess(result)) {
    return Response.json({ error: 'Analytics service unavailable' }, { status: 500 });
  }

  return Response.json(toDashboardSessionsResponse(result), { status: 200 });
}
