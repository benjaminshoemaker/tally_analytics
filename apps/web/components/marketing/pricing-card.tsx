...
export default function PricingCard(props: PricingCardProps) {
  const ctaClassName = `group mt-6 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold transition-all active:scale-[0.98] ${
    props.highlighted
      ? 'bg-[#0f766e] text-white shadow-warm hover:bg-teal-800 hover:shadow-warm-md active:shadow-none'
    ...
    >
      {props.highlighted && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2">
          <span
            className="inline-flex items-center gap-1.5 rounded-full bg-[#0f766e] px-3 py-1 text-xs font-semibold text-white shadow-warm animate-pulse"
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="text-right">
          <div className="font-display text-3xl tracking-tight text-[#1b140d]">
            {props.priceLabel}
          </div>
          <div className="text-sm text-[#57534e]">{props.priceSuffix}</div>
        </div>
      </div>

      <dl className="mt-6 grid gap-2 text-sm">
        <div className="flex items-center justify-between gap-4 rounded-lg bg-[#f3ede7]/50 px-3 py-2">
          <dt className="flex items-center gap-2 text-[#57534e]">
        ...
        <div className="flex items-center justify-between gap-4 px-3 py-2">
          ...
          <dd
            className={`font-medium ${isUnlimited(props.projectsLabel) ? 'text-brand-600' : 'text-[#1b140d]'}`}
          >
            {props.projectsLabel}
          </dd>
        </div>
        <div className="flex items-center justify-between gap-4 rounded-lg bg-[#f3ede7]/50 px-3 py-2">
          ...
          <dd
            className={`font-medium ${isUnlimited(props.retentionLabel) ? 'text-brand-600' : 'text-[#1b140d]'}`}
          >
        ...
        <div className="flex items-center justify-between gap-4 px-3 py-2">
          ...
          <dd
            className={`font-medium ${isPriority(props.supportLabel) ? 'text-brand-600' : 'text-[#1b140d]'}`}
          >
            {props.supportLabel}
          </dd>
        </div>
