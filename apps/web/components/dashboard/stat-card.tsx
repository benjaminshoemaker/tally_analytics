  ...
  return (
    <div className="group relative overflow-hidden rounded-lg border border-warm-200 bg-white p-4 shadow-warm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-warm-md">
      ...
      <div className="mt-2 flex items-baseline justify-between gap-3">
        <p className="min-w-0 truncate font-display text-2xl font-semibold text-warm-900">
          {value}
        </p>
        {changeLabel && (
          <p
            className={`flex items-center gap-1 text-sm font-medium ${
              isPositive ? 'text-emerald-700' : isNegative ? 'text-red-700' : 'text-warm-500'
            }`}
          >
            {isPositive && (
      ...
      {detail && <p className="mt-2 truncate text-xs text-warm-500">{detail}</p>}
    </div>
  );
}
