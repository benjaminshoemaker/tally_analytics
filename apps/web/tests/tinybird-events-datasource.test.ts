...
describe("tinybird events datasource (Task 4.4.A)", () => {
  ...
  it("defines the events datasource schema + retention", () => {
    ...
    expect(contents).toContain("`session_id` String");
    expect(contents).toContain("`event_type` LowCardinality(String)");
    expect(contents).toContain("`timestamp` DateTime64(3)");
    expect(contents).toContain("`country` Nullable(String)");
    expect(contents).toContain("`city` Nullable(String)");
